...
to a ESP32 S3 DevkitC-1 controller board (the board is connected and powered by the 3D Printer (e.g. Anycubic Photon D2) via a 
USBC to USBA data cable plugged into the LEFT hand USB type C connector (Marked as 'USB' on the back of the ESP32).
...
called **WIFI** in the root of the SD Card. 
The code will first check this file and if found will use these credentials - if not found then 
it will use whatever is hard coded in the main tab.
...
If SD Card initialisation fails - try reducing the frequency in line 310 (e.g. 10000 or 20000)
